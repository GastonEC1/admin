...
const ActivoList = () => {
  const API_BASE_URL = "https://gestion-3kgo.onrender.com/api";
  ...
  useEffect(() => {
    ...
    const obtenerActivos = async () => {
      ...
      try {
        ...
        const uniqueTypes = [
          "Todos",
          ...new Set(data.map((activo) => activo.tipo)),
        ];
        ...
        setError(
          "No se pudo cargar la lista. Revisa la conexión con el servidor."
        );
      } finally {
        setCargando(false);
      }
  ...
  const filteredActivos = activos.filter((activo) => {
    ...
    const matchesSelectedType =
      !selectedTipo || selectedTipo === "Todos" || activo.tipo === selectedTipo;
  ...
  const currentItems = filteredActivos.slice(
    startIndex,
    startIndex + itemsPerPage
  );

  if (cargando)
    return <div className="text-center mt-5">Cargando lista...</div>;
  if (error)
  ...
  return (
    <div className="container mt-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1>
          {consorcioId ? "Activos del Consorcio" : "Selecciona un Consorcio"}
        </h1>
      ...
      {consorcioId && (
        <div className="d-flex flex-column flex-md-row mb-3">
          ...
          <div className="input-group">
            <label className="input-group-text" htmlFor="tipoFilter">
              Tipo
            </label>
            <select
              id="tipoFilter"
      ...
      {consorcioId ? (
            ...
            currentItems.map((activo) => (
              ...
              >
                <Link
                  ...
                  className="text-decoration-none text-dark d-flex justify-content-between align-items-center w-100"
                >
        ...
        <div className="alert alert-info text-center mt-5">
          Por favor, selecciona un consorcio de la página anterior para ver sus
          activos.
        </div>
      )}
      {totalPages > 1 && (
