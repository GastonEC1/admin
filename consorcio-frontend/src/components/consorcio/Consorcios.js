...
// Placeholder icons to avoid a dependency on `react-icons/fa`.
const FaTrash = () => <span>&#x1F5D1;</span>;
const FaEdit = () => <span>&#x270E;</span>;
const FaRegBuilding = () => <span>&#x1F3E2;</span>;
...
function Consorcios() {
    ...
    const currentItems = filteredConsorcios.slice(startIndex, startIndex + itemsPerPage);
    ...
    return (
        <Container className="my-5">
            <Card className="shadow-lg border-0 rounded-4 p-4">
                ...
                {currentItems.length > 0 ? (
                                ...
                                {currentItems.map(consorcio => (
                                    <tr key={consorcio._id}>
                                        <td className="align-middle">
                                            <Link to={`/consorcios/${consorcio._id}`} className="fw-bold text-decoration-none text-primary">
                                                {consorcio.nombre}
                                            </Link>
                                        </td>
                    ...
                    <Alert variant="info" className="mt-3 text-center shadow-sm border-0">
                        {searchTerm ? 'No se encontraron consorcios que coincidan con la búsqueda.' : 'No hay consorcios para mostrar. Crea uno nuevo para empezar.'}
                    </Alert>
                )}
            </Card>
            ...
            <Modal show={showConfirmModal} onHide={handleCloseConfirm} centered>
                ...
                <Modal.Body>
                    ¿Estás seguro de que quieres eliminar el consorcio <strong>{consorcioToDelete?.nombre}</strong> y toda su información asociada? Esta acción es irreversible.
                </Modal.Body>
                <Modal.Footer>
