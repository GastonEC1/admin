...
function ConsorcioDetail({ API_BASE_URL, userRole }) {
  ...
  const currentActivos = activos.slice(
    startIndexActivos,
    startIndexActivos + itemsPerPageActivos
  );
  ...
  useEffect(() => {
    const filtrados = inquilinos.filter((inquilino) => {
      ...
      const apellido = inquilino.apellido
        ? inquilino.apellido.toLowerCase()
        : "";
      const term = searchTerm.toLowerCase();
      const matchesSearchTerm =
        !searchTerm || nombre.startsWith(term) || apellido.startsWith(term);
      const matchesSelectedTipo =
        !selectedTipoInquilino ||
        selectedTipoInquilino === "Todos" ||
        inquilino.tipo === selectedTipoInquilino;
      return matchesSearchTerm && matchesSelectedTipo;
    });
    setFilteredInquilinos(filtrados);
  }, [inquilinos, searchTerm, selectedTipoInquilino]);

  const totalPagesInquilinos = Math.ceil(
    filteredInquilinos.length / itemsPerPageInquilinos
  );
  const startIndexInquilinos =
    (currentPageInquilinos - 1) * itemsPerPageInquilinos;
  const currentInquilinos = filteredInquilinos.slice(
    startIndexInquilinos,
    startIndexInquilinos + itemsPerPageInquilinos
  );
  ...
  const tiposInquilino = [
    "Todos",
    ...new Set(inquilinos.map((i) => i.tipo || "")),
  ];
  ...
  const handleSaveEvento = async () => {
    ...
    if (modalMode === "add") {
      await axios.post(`${API_BASE_URL}/api/calendars`, eventoAEnviar, {
        headers: { "x-auth-token": token },
      });
    } else {
      await axios.put(
            ...
            {totalPagesInquilinos > 1 && (
              <div className="d-flex justify-content-center flex-wrap my-2">
                <Pagination>
                  <Pagination.Prev
                    onClick={() =>
                      setCurrentPageInquilinos(currentPageInquilinos - 1)
                    }
            ...
            {totalPagesActivos > 1 && (
              <div className="d-flex justify-content-center flex-wrap my-2">
                <Pagination>
                  <Pagination.Prev
                    onClick={() =>
                      setCurrentPageActivos(currentPageActivos - 1)
                    }
