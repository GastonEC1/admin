...
function EditInquilino() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const inquilinoResponse = await axios.get(
          `${inquilinosBackendUrl}/${id}`,
          {
        ...
        setErrorMessage(
          "Error al cargar los datos del inquilino o consorcios."
        );
        setLoading(false);
        console.error(err);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMessage(
        "Error al actualizar el inquilino. Por favor, revisa los datos."
      );
